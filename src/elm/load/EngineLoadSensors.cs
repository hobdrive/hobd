
/*********************************************************
 * AUTOGENERATED with ecuxml2cs.xsl!
 * DO NOT EDIT WITH HANDS!
 *********************************************************/
namespace hobd{

using System;

public class EngineLoadSensors : SensorProvider
{

    public EngineLoadSensors()
    {
    }
    
    public string GetName()
    {
        return "EngineLoadSensors";
    }

    public string GetDescription()
    {
        return "LitersPerHour calculated sensor";
    }
    
    public string GetDescription(string lang)
    {
        return GetDescription();
    }

    public void Activate(SensorRegistry registry)
    {
        CoreSensor s;
        
        // EngineLoadLitersPerHour
        s = new hobd.elm.load.LitersPerHourSensor()
            {
                ID = "EngineLoad.LitersPerHour",
                Name = "LitersPerHour",
                
            };
        
        s.Units = "lphour";
  
        registry.Add(s);

    }

    
  public const string LitersPerHour = "LitersPerHour";

}

}
/**
  List of PIDs generated:

  - LitersPerHour - Fuel Per Hour - Fuel Per Hour (Rough engine load based). Correction coefficient is required! - lphour
*/
